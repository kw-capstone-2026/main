...
print("=== Base expected value E[f(x)] ===")
print(explainer.expected_value)

if not danger_candidates.empty:
    idx = danger_candidates.index[0]
    loc = np.where(X_sample.index == idx)[0][0]
    print(f"\n=== HIGH RISK STORE (Danger) Index: {idx}, Prob: {danger_candidates.loc[idx, 'predicted_prob']:.4f} ===")
    print(f"f(x) value (sum of SHAP + base): {shap_values[loc].values.sum() + explainer.expected_value:.4f}")
    
    # Sort features by absolute SHAP value for this instance
    sh_val = shap_values[loc].values
    feat_val = X_sample.iloc[loc].values
    feat_names = X_sample.columns
    
    sorted_idx = np.argsort(np.abs(sh_val))[::-1]
    print("\nFeature Contributions:")
    for i in sorted_idx[:15]:
        print(f"  {feat_names[i]} = {feat_val[i]} | SHAP: {sh_val[i]:+.4f}")

if not normal_candidates.empty:
    idx = normal_candidates.index[0]
    loc = np.where(X_sample.index == idx)[0][0]
    print(f"\n=== SAFE STORE (Normal) Index: {idx}, Prob: {normal_candidates.loc[idx, 'predicted_prob']:.4f} ===")
    print(f"f(x) value (sum of SHAP + base): {shap_values[loc].values.sum() + explainer.expected_value:.4f}")
